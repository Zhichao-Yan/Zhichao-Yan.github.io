...
2. 3. 默认的目标通常是 default.target，它是一个符号链接，指向实际的目标单元文件，如`graphical.target`或`multi-user.target`，这些目标定义了系统启动时需要达到的状态


2. 读取 `/etc/systemd/system/` 和 `/usr/lib/systemd/system/ `下的单元文件（如`sshd.service`），通过加载这些单元文件，systemd 能够了解需要启动的服务和资源，以及它们之间的依赖关系。
...
3. 解决依赖关系并构建启动顺序：基于单元文件中声明的依赖项（如`Requires/Wants/After/Before`），`systemd`构建一个有向无环图（`DAG`）来确定最佳的启动顺序
    * `.service`： 定义服务的核心配置，包括如何启动、停止、重启服务，以及依赖关系、环境变量等,直接控制服务的运行行为
    * `.socket`： 定义套接字激活的服务，允许服务在需要时才启动
...
4. 启动服务和其他资源： 根据计算出的最佳启动顺序，并行地启动各个服务和资源。对于那些没有依赖关系的服务，systemd 可以同时启动它们以加快启动速度，这不仅提高了系统的启动效率，还保证了每个服务都能在所需的依赖项准备好之后才开始运行

4. 日志集成：通过 `journalctl` 查看启动日志


* `Systemd`负责后续的系统启动流程，包括启动必要的服务、挂载文件系统以及提供登录提示符等
* `Systemd`负责管理系统中的各种服务、设备、套接字等资源
* `Systemd`引入了“目标”（targets）的概念来替代传统的运行级别（runlevels），提供了更加灵活和细粒度的控制机制。
